	...
	jQuery(function($){
		...
		function sendFileToServer(formData,status) {
			...
			var jqXHR = $.ajax({
				...
				success: function(data){
					...
					$("#texture-upload-status").append("File upload Done<br>");         
				}
			}); 
		...
		*/
		...
		function handleFileUpload(files) {
			var supplier = $('#upload_to_supplier').val(),
				design_type = $('#upload_to_design_type').val(),
				material = $('#upload_to_material').val(),
				texture_product = $('#upload_to_texture_product').val(),
				segment = $('#upload_to_segment').val(),
			...
			for (var i = 0; i < files.length; i++) {
				...
				fd.append('texture_product', texture_product);
